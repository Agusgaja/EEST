    ...
    <div className={`${theme === "dark" ? "dark" : ""}`}>
      <div className="ambient-bg flex h-screen w-screen overflow-hidden transition-colors p-3 sm:p-4 md:p-5 gap-3 sm:gap-4 md:gap-5">
        ...
        <div className="flex flex-1 flex-col overflow-hidden relative rounded-2xl border border-slate-200/60 bg-white/60 shadow-xl dark:border-white/10 dark:bg-slate-900/40 dark:shadow-2xl dark:backdrop-blur-xl backdrop-blur-md">
          <header className="flex min-h-[77px] items-center justify-between border-b border-slate-200/50 px-6 transition-colors dark:border-white/10">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setIsMobileOpen(true)}
                className="md:hidden flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-slate-200/80 bg-white/40 text-slate-500 shadow-sm transition-colors hover:bg-slate-50 dark:border-white/10 dark:bg-slate-800/40 dark:text-slate-400 dark:hover:bg-white/10"
              >
                <Menu size={20} />
              </button>
